...
public class BlazearmEntity extends PersistentProjectileEntity {
    ...
    public BlazearmEntity(EntityType<? extends BlazearmEntity> entityType, World world) {
        super((EntityType<? extends PersistentProjectileEntity>)entityType, world, DEFAULT_STACK);
    }

    public BlazearmEntity(World world, LivingEntity owner, ItemStack stack) {
        super(ModEntityTypes.BLAZEARM_ENTITY, owner, world, stack);
    ...
    public BlazearmEntity(World world, double x, double y, double z, ItemStack stack) {
        super(ModEntityTypes.BLAZEARM_ENTITY, x, y, z, world, stack);
        this.dealtDamage = false;
        this.returnTimer = 0;
        this.impactFactor = 1.0f;
        this.dataTracker.set(LOYALTY, (byte)EnchantmentHelper.getLevel(Enchantments.LOYALTY, stack));
    ...
    protected void onEntityHit(EntityHitResult entityHitResult) {
        ...
        if (entity != null) {
            ...
            if (entity instanceof LivingEntity) {
                ...
                f += !livingEntity.getType().isIn(ModEntityTypeTags.SENSITIVE_TO_LEANING) && (livingEntity.isOnFire() || livingEntity.isInLava() || entityMagmaBlockState.isOf(Blocks.MAGMA_BLOCK)
                        || (livingEntity.hasStatusEffect(StatusEffects.FIRE_RESISTANCE) && !livingEntity.isWet())) ?
                        1.0f*EnchantmentHelper.getLevel(ModEnchantments.LEANING, this.getItemStack()) : 0.0f;
            ...
            if (entity.damage(damageSource, f)) {
                ...
                if (entity instanceof LivingEntity) {
                    ...
                    if (!livingEntity2.isOnFire()) {
                        livingEntity2.setOnFireFor(8*(1 + EnchantmentHelper.getLevel(Enchantments.FIRE_ASPECT, this.getItemStack())) + this.random.nextBetween(0, 8));
                    }
