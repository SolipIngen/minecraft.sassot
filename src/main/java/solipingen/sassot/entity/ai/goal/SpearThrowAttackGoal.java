...
public class SpearThrowAttackGoal extends ProjectileAttackGoal {
    ...
    public void tick() {
        ...
        if (--this.updateCountdownTicks == 0) {
            ...
            if (d <= (double)this.squaredMaxShootRange && !this.mob.isInAttackRange(this.target)) {
            ...
            else if (this.mob.isInAttackRange(this.target)) {
                this.mob.swingHand(Hand.MAIN_HAND);
                this.mob.tryAttack(this.target);
            }
        } 
        else if (this.updateCountdownTicks < 0) {
            this.updateCountdownTicks = this.mob.isInAttackRange(this.target) ? 20 : MathHelper.floor(MathHelper.lerp(Math.sqrt(d) / (double)this.maxShootRange, (double)this.minIntervalTicks, (double)this.maxIntervalTicks));
        }
    }
}
