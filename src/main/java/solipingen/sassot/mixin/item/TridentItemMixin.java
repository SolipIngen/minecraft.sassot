...
public class TridentItemMixin extends Item {
    ...
    private static void injectedAttributeModifiers(CallbackInfoReturnable<AttributeModifiersComponent> cbireturn) {
        cbireturn.setReturnValue(AttributeModifiersComponent.builder()
                .add(EntityAttributes.GENERIC_ATTACK_DAMAGE,
                        new EntityAttributeModifier(Item.BASE_ATTACK_DAMAGE_MODIFIER_ID, 9.0, EntityAttributeModifier.Operation.ADD_VALUE), AttributeModifierSlot.MAINHAND)
                .add(EntityAttributes.GENERIC_ATTACK_SPEED,
                        new EntityAttributeModifier(Item.BASE_ATTACK_SPEED_MODIFIER_ID, -2.4f, EntityAttributeModifier.Operation.ADD_VALUE), AttributeModifierSlot.MAINHAND)
    ...
    private void injectedOnStoppedUsing(ItemStack stack, World world, LivingEntity user, int remainingUseTicks, CallbackInfo cbi) {
        if (world instanceof ServerWorld serverWorld) {
            ...
            int j = EnchantmentHelper.getLevel(enchantmentLookup.getOrThrow(Enchantments.RIPTIDE), stack);
            if (user instanceof PlayerEntity playerEntity && j > 0) {
                ...
                List<Entity> entityList = serverWorld.getOtherEntities(playerEntity, riptideBox);
                for (Entity otherEntity : entityList) {
                    ...
                    if (otherEntity instanceof LivingEntity livingOtherEntity) {
                        float damageAmount = EnchantmentHelper.getDamage(serverWorld, stack, livingOtherEntity, playerEntity.getDamageSources().playerAttack(playerEntity), 9.0f);
                        if (stack.isOf(Items.TRIDENT) && !livingOtherEntity.getType().isIn(EntityTypeTags.SENSITIVE_TO_IMPALING) && livingOtherEntity.isWet()) {
                            damageAmount += 1.0f*EnchantmentHelper.getLevel(enchantmentLookup.getOrThrow(Enchantments.IMPALING), stack);
                        }
