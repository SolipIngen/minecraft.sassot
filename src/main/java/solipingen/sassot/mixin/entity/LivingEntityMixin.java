...
public abstract class LivingEntityMixin extends Entity implements LivingEntityInterface {
    ...
    private static final TrackedData<String> SKEWERING_ENTITY = DataTracker.registerData(LivingEntity.class, TrackedDataHandlerRegistry.STRING);
    ...
    @Nullable private PersistentProjectileEntity skeweringEntity;
    private boolean isUsingWhirlwind;
    private boolean isUsingFlare;

    @Invoker("attackLivingEntity")
    public abstract void invokeAttackLivingEntity(LivingEntity target);
    ...
    private void injectedTickMovement(CallbackInfo cbi) {
        if (this.canBeSkewered() && this.getIsSkewered() && this.skeweringEntity != null) {
            ...
            if (this.skeweringEntity instanceof SpearEntity) {
                SpearEntity spearEntity = (SpearEntity)this.skeweringEntity;
            ...
            else if (this.skeweringEntity instanceof TridentEntity) {
                TridentEntity tridentEntity = (TridentEntity)this.skeweringEntity;
            ...
            else if (this.skeweringEntity instanceof BlazearmEntity) {
                BlazearmEntity blazearmEntity = (BlazearmEntity)this.skeweringEntity;
            ...
            double unstuckThresholdD = 1.0 / (1.0 + Math.exp(-((double)(this.getWorld().getTime() - this.lastDamageTime) - 200.0)));
            if (unstuckRandomD < unstuckThresholdD && this.skeweringEntity != null) {
                if (this.damage(this.getDamageSources().trident(this.skeweringEntity, null), 2.0f)) {
                    this.setIsSkewered(false);
                    this.setPos(this.getX(), this.getY() + 1.0, this.getZ());
                    this.onDamaged(this.getDamageSources().trident(this.skeweringEntity, null));
                    this.setSkeweringEntity(null);
                }
            }
            else if (unstuckRandomD >= unstuckThresholdD && this.skeweringEntity != null && this.skeweringEntity.isRemoved()) {
                if (this.damage(this.getDamageSources().trident(this.skeweringEntity, null), 2.0f)) {
                    this.setIsSkewered(false);
                    this.setPos(this.getX(), this.getY() + 1.0, this.getZ());
                    this.onDamaged(this.getDamageSources().trident(this.skeweringEntity, null));
    ...
    private void injectedTickRiptide(Box a, Box b, CallbackInfo cbi) {
        Box box = a.union(b);
        List<Entity> list = this.getWorld().getOtherEntities(this, box);
        if (!list.isEmpty()) {
            for (Entity entity : list) {
                if (!(entity instanceof LivingEntity)) continue;
                this.invokeAttackLivingEntity((LivingEntity)entity);
                this.riptideTicks = 0;
        ...
        if (!this.getWorld().isClient && this.riptideTicks <= 0) {
            this.invokeSetLivingFlag(4, false);
        }
    }
    ...
    public void setIsUsingTridentRiptide(boolean usingRiptide) {
        this.invokeSetLivingFlag(4, usingRiptide);
    }
