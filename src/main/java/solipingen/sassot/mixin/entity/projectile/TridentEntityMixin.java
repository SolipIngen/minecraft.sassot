...
public abstract class TridentEntityMixin extends PersistentProjectileEntity implements TridentEntityInterface {
    ...
    private float injectedAttackDamage(float f, EntityHitResult entityHitResult) {
        ...
        float additionalDamage = EnchantmentHelper.getAttackDamage(this.getItemStack(), EntityGroup.DEFAULT);
        if (targetEntity.isWet()) {
            if (targetEntity instanceof LivingEntity) {
                LivingEntity livingTargetEntity = (LivingEntity)targetEntity;
                additionalDamage = EnchantmentHelper.getAttackDamage(this.getItemStack(), livingTargetEntity.getGroup());
                return baseDamage + additionalDamage + 1.25f*EnchantmentHelper.getLevel(Enchantments.IMPALING, this.getItemStack());
            }
            return baseDamage + additionalDamage + 1.25f*EnchantmentHelper.getLevel(Enchantments.IMPALING, this.getItemStack());
        }
        ...
        if (entity instanceof LivingEntity) {
            ...
            if (this.hasSkewering()) {
                ...
                if (this.getOwner() instanceof ServerPlayerEntity) {
                    ...
                    float f = 9.0f + EnchantmentHelper.getAttackDamage(this.getItemStack(), EntityGroup.DEFAULT);
