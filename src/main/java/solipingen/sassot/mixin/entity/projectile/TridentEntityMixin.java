...
public abstract class TridentEntityMixin extends PersistentProjectileEntity implements TridentEntityInterface {
    ...
    private float injectedAttackDamage(float f, EntityHitResult entityHitResult) {
        ...
        if (targetEntity.isWet()) {
            if (targetEntity instanceof LivingEntity) {
                ...
                additionalDamage = EnchantmentHelper.getAttackDamage(this.tridentStack, livingTargetEntity.getGroup());
                return baseDamage + additionalDamage + 1.25f*EnchantmentHelper.getLevel(Enchantments.IMPALING, this.tridentStack);
            }
            return baseDamage + additionalDamage + 1.25f*EnchantmentHelper.getLevel(Enchantments.IMPALING, this.tridentStack);
        }
        ...
        if (entity instanceof LivingEntity) {
            ...
            if (this.hasSkewering()) {
                ...
                if (this.getOwner() instanceof ServerPlayerEntity) {
                    ...
                    float f = 9.0f + EnchantmentHelper.getAttackDamage(this.tridentStack, EntityGroup.DEFAULT);
    ...
    public boolean hasSkewering() {
        return EnchantmentHelper.getLevel(ModEnchantments.SKEWERING, this.tridentStack) > 0;
    }
