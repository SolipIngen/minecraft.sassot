...
public class SpearItem extends Item implements ProjectileItem {
    ...
    public SpearItem(ToolMaterial material, Item.Settings settings) {
        super(settings.maxDamage(material.getDurability()).component(DataComponentTypes.TOOL, SpearItem.createToolComponent()));
    ...
    public static AttributeModifiersComponent createAttributeModifiers(ToolMaterial material, float baseAttackDamage, float attackSpeed) {
        return AttributeModifiersComponent.builder()
                .add(EntityAttributes.GENERIC_ATTACK_DAMAGE,
                        new EntityAttributeModifier(ATTACK_DAMAGE_MODIFIER_ID, "Weapon modifier", baseAttackDamage + material.getAttackDamage(), EntityAttributeModifier.Operation.ADD_VALUE), AttributeModifierSlot.MAINHAND)
                .add(EntityAttributes.GENERIC_ATTACK_SPEED,
                        new EntityAttributeModifier(ATTACK_SPEED_MODIFIER_ID, "Weapon modifier", attackSpeed, EntityAttributeModifier.Operation.ADD_VALUE), AttributeModifierSlot.MAINHAND)
                .add(EntityAttributes.PLAYER_BLOCK_INTERACTION_RANGE,
                        new EntityAttributeModifier(REACH_MODIFIER_ID, "Tool modifier", 1.0, EntityAttributeModifier.Operation.ADD_VALUE), AttributeModifierSlot.MAINHAND)
                .add(EntityAttributes.PLAYER_ENTITY_INTERACTION_RANGE,
                        new EntityAttributeModifier(ATTACK_RANGE_MODIFIER_ID, "Weapon modifier", 1.0, EntityAttributeModifier.Operation.ADD_VALUE), AttributeModifierSlot.MAINHAND)
                .build();
    }
    ...
    public void onStoppedUsing(ItemStack stack, World world, LivingEntity user, int remainingUseTicks) {
        ...
        if (!world.isClient) {
            ...
            if (j == 0) {
                ...
                spearEntity.setPierceLevel((byte)EnchantmentHelper.getLevel(Enchantments.PIERCING, stack));
    ...
    public ProjectileEntity createEntity(World world, Position pos, ItemStack stack, Direction direction) {
        SpearEntity spearEntity = new WoodenSpearEntity(world, pos.getX(), pos.getY(), pos.getZ(), stack.copyWithCount(1));
        if (stack.isOf(ModItems.BAMBOO_SPEAR)) {
            spearEntity = new BambooSpearEntity(world, pos.getX(), pos.getY(), pos.getZ(), stack.copyWithCount(1));
        }
        else if (stack.isOf(ModItems.STONE_SPEAR)) {
            spearEntity = new StoneSpearEntity(world, pos.getX(), pos.getY(), pos.getZ(), stack.copyWithCount(1));
        }
        else if (stack.isOf(ModItems.FLINT_SPEAR)) {
            spearEntity = new FlintSpearEntity(world, pos.getX(), pos.getY(), pos.getZ(), stack.copyWithCount(1));
        }
        else if (stack.isOf(ModItems.COPPER_SPEAR)) {
            spearEntity = new CopperSpearEntity(world, pos.getX(), pos.getY(), pos.getZ(), stack.copyWithCount(1));
        }
        else if (stack.isOf(ModItems.GOLDEN_SPEAR)) {
            spearEntity = new GoldenSpearEntity(world, pos.getX(), pos.getY(), pos.getZ(), stack.copyWithCount(1));
        }
        else if (stack.isOf(ModItems.IRON_SPEAR)) {
            spearEntity = new IronSpearEntity(world, pos.getX(), pos.getY(), pos.getZ(), stack.copyWithCount(1));
        }
        else if (stack.isOf(ModItems.DIAMOND_SPEAR)) {
            spearEntity = new DiamondSpearEntity(world, pos.getX(), pos.getY(), pos.getZ(), stack.copyWithCount(1));
        }
        else if (stack.isOf(ModItems.NETHERITE_SPEAR)) {
            spearEntity = new NetheriteSpearEntity(world, pos.getX(), pos.getY(), pos.getZ(), stack.copyWithCount(1));
        }
