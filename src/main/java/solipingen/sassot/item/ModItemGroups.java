...
public class ModItemGroups {
    ...
    public static void registerModItemsToVanillaGroups() {
        ...
        ItemGroupEvents.modifyEntriesEvent(ItemGroups.INGREDIENTS).register(entries -> {
            Optional<RegistryWrapper.Impl<Enchantment>> registryWrapperOptional = entries.getContext().lookup().getOptionalWrapper(RegistryKeys.ENCHANTMENT);
            registryWrapperOptional.ifPresent(registryWrapper -> {
                registryWrapper.streamEntries().map(RegistryEntry::value)
                        .filter(enchantment -> enchantment.isEnabled(entries.getEnabledFeatures()))
                        .filter(enchantment -> modEnchantableTagsSet.contains(enchantment.getApplicableItems()))
                        .map(enchantment -> EnchantedBookItem.forEnchantment(new EnchantmentLevelEntry((Enchantment)enchantment, enchantment.getMaxLevel())))
